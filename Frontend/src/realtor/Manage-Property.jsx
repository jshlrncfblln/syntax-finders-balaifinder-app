...
export default function ManageProperty(){
    ...
    const handleAddProperty = async (property) => {
      try {
        const response = await axios.post(`${backendurl}/api/post/crud/addproperties`, property);
        console.log('Add Property Response:', response.data);
        closeAddModal();
        // Optionally, you can refetch the properties after adding a new one
        getProperties();
      } catch (error) {
    ...
    const deleteProperty = async (id) => {
        try {
          await axios.delete(`${backendurl}/api/post/crud/delpropeties/${id}`); // Assuming your API endpoint for deleting a property is '/api/properties/:id'
                        ...
                        {properties.map(property => (
                            ...
                            <td class="py-4 px-6 border-b border-gray-200 text-center items-center justify-center">
                                <div class="flex gap-1">
                                    ...
                                    <div class="flex-1">
                                    <button onClick={() => deleteProperty(property.id)} className="bg-red-500 hover:bg-red-700 text-white py-1 px-2 rounded-md text-xs sm:text-sm md:text-sm">Delete</button>
                                    </div>
                                </div>
                            </td>
                        </tr>
